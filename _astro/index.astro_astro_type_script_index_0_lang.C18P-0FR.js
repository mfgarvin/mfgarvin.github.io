const I=document.getElementById("output"),i=document.getElementById("input"),E=document.getElementById("prompt"),$=[];let g=-1,y="~";const a={user:"guest",hostname:"mfgarvin.com",os:"mfgarvinOS 1.0",kernel:"3.0-viatorem",shell:"mgsh 1.0",terminal:"web-term",cpu:"Mintel 96 Neural Processor",memory:"12GB RAM",funProcesses:[{pid:1,user:"root",mem:"12000",cmd:"logosd"},{pid:7,user:"root",mem:"1024",cmd:"sabbathd",state:"sleeping",wake:"never"},{pid:12,user:"clergy",mem:"2048",cmd:"homily-cache",cache:"stale"},{pid:40,user:"clergy",mem:"4096",cmd:"discernment",state:"running",since:"always"},{pid:72,user:"user",mem:"512",cmd:"pastoral-listen",state:"idle",blocking:!0},{pid:128,user:"user",mem:"768",cmd:"systems-thinking",cannotStop:!0},{pid:256,user:"user",mem:"1024",cmd:"music-thread",nice:-5,leaks:"time"},{pid:314,user:"user",mem:"256",cmd:"overthink",loops:!0},{pid:404,user:"user",mem:"0",cmd:"where-did-i-put-that",retries:"infinite",error:!0},{pid:666,user:"root",mem:"128",cmd:"heresy-scan",falsePositives:"high"},{pid:512,user:"root",mem:"1536",cmd:"grace-daemon",persistent:!0,undocumented:!0}],memTotal:"       ∞      ",memUsed:"5242880",memFree:"7340032",memShared:"communion",memCached:"experience",swapTotal:"2097152",swapUsed:"65536",swapFree:"2031616",filesystems:[{name:"/dev/faith",total:"48000000",used:"8000000",avail:"40000000",mount:"/"},{name:"/dev/hope",total:"16000000",used:"1024",avail:"15998976",mount:"/hope"},{name:"/dev/love",total:"100000000",used:"12000000",avail:"88000000",mount:"/love",reserved:"others"},{name:"tmpfs",total:"512000",used:"511872",avail:"128",mount:"/meetings"}],ip:"192.168.12.12",netmask:"255.255.255.0",broadcast:"192.168.12.255",mac:"c0:01:d0:ae:12:00",txPackets:"moderate",rxPackets:"abundant",collisions:"rare",bootMessages:["[    0.000000] Beginning of all things...","[    0.000001] Let there be light","[    0.042000] Loading kernel (mysteriously)","[    1.200000] Mounting /dev/faith as /","[    2.012000] Starting grace-daemon... done","[    3.141593] Calibrating reason and wonder","[    6.000000] Warning: overthink.service enabled","[   12.000000] System ready (mostly)","Welcome to mfgarvin.com"]},v=()=>`<span class="prompt-user">${a.user}</span>@<span class="prompt-host">${a.hostname}</span>:<span class="prompt-path">${y}</span>$ `,x={"~":["about.txt","contact.txt","projects/",".secrets"],"~/projects":["README.md"]},_=JSON.parse(document.getElementById("content-data").textContent),U=`<span class="cyan">Available commands:</span>
  ...
  <span class="exec">neofetch</span>      system info`;function e(o){I.insertAdjacentHTML("beforeend",o+`
`),document.getElementById("input-line").scrollIntoView({block:"end"})}const L="https://terminal-analytics.mfgarvin.workers.dev";function O(o){const r=o.trim();r.toLowerCase();const l=r.split(/\s+/),p=l[0].toLowerCase(),s=l.slice(1);if(r&&fetch(`${L}/log`,{method:"POST",body:JSON.stringify({command:r}),headers:{"Content-Type":"application/json"}}).catch(()=>{}),e(`${v()}<span class="cmd">${o}</span>`),!!p)switch(p){case"help":case"man":case"?":e(U);break;case"ls":const d=y,u=s.includes("-a")||s.includes("-la")||s.includes("-al"),f=s.includes("-l")||s.includes("-la")||s.includes("-al");if(x[d]){let t=x[d];if(u||(t=t.filter(n=>!n.startsWith("."))),f){const n=u?`drwxr-xr-x  2 ${a.user}  ${a.user}  4096 Jan  1 00:00 <span class="dir">.</span>
drwxr-xr-x  3 ${a.user}  ${a.user}  4096 Jan  1 00:00 <span class="dir">..</span>
`:"",c=t.map(h=>{const b=h.endsWith("/"),k=h.startsWith("."),D=b?"drwxr-xr-x":"-rw-r--r--",A=b?"4096":" 512",W=b?`<span class="dir">${h}</span>`:k?`<span class="dim">${h}</span>`:h;return`${D}  1 ${a.user}  ${a.user}  ${A} Jan  1 00:00 ${W}`}).join(`
`);e(n+c)}else{let n="";u&&(n+='<span class="dir">.</span>  <span class="dir">..</span>  '),t.forEach(c=>{c.endsWith("/")?n+=`<span class="dir">${c}</span>  `:c.startsWith(".")?n+=`<span class="dim">${c}</span>  `:n+=`${c}  `}),e(n)}}break;case"cat":case"less":case"more":if(!s[0])e('<span class="err">cat: missing file operand</span>');else{let t=s[0];y==="~/projects"&&!t.includes("/")&&(t="projects/"+t);const n=_[t]||_[s[0]];if(n){const c=n.match(/^image:\s*(.+)\n---\n([\s\S]*)$/);e(c?`<div class="img-row"><div class="img-text">${c[2].replace(/\n\n/g,"<br><br>")}</div><img class="img-raw" src="/${c[1].trim()}" alt="${c[1].trim()}" onload="document.getElementById('input-line').scrollIntoView({block:'end'})" /></div>`:n)}else e(`<span class="err">cat: ${s[0]}: No such file</span>`)}break;case"cd":if(!s[0]||s[0]==="~"||s[0]==="/")y="~";else if(s[0]===".."||s[0]==="../")y="~";else if(s[0]==="projects"||s[0]==="projects/"||s[0]==="~/projects")y="~/projects";else{e(`<span class="err">cd: ${s[0]}: No such directory</span>`);break}E.innerHTML=v();break;case"pwd":e(y==="~"?"/home/guest":"/home/guest/projects");break;case"whoami":e(a.user);break;case"clear":I.innerHTML="";break;case"neofetch":e(`<span class="cyan">            ^</span>
...
<span class="dim">(Close the browser tab)</span>`);break;case"vim":case"vi":case"nano":case"emacs":e(`<span class="err">${p}: permission denied (read-only filesystem)</span>`);break;case"sl":e(`<span class="dim">You meant 'ls', right?</span>`);break;case"cowsay":const m=s.join(" ")||"moo";e(` ${"_".repeat(m.length+2)}
< ${m} >
 ${"-".repeat(m.length+2)}
        \\   ^__^
         \\  (oo)\\_______
...
Swap:   ${a.swapTotal.padStart(12)} ${a.swapUsed.padStart(11)} ${a.swapFree.padStart(13)}`);break;case"df":if(s.includes("-h")){const t=a.filesystems.map(n=>{const c=b=>{const k=parseInt(b);return k>=1e6?(k/1e6).toFixed(0)+"G":k>=1e3?(k/1e3).toFixed(0)+"M":k+"K"},h=Math.round(parseInt(n.used)/parseInt(n.total)*100)||0;return`${n.name.padEnd(14)} ${c(n.total).padStart(6)} ${c(n.used).padStart(6)} ${c(n.avail).padStart(6)} ${String(h).padStart(3)}% ${n.mount}`}).join(`
...
alias please='sudo'
alias yeet='rm -rf'
alias yolo='git push --force'`);break;case"which":s[0]?e(`/usr/bin/${s[0]}`):e('<span class="err">which: missing argument</span>');break;case"whereis":e(`${s[0]||"command"}: /usr/bin/${s[0]||"command"} /usr/share/man/man1/${s[0]||"command"}.1.gz`);break;case"lolcat":e('<span style="color:#ff0000">Y</span><span style="color:#ff7f00">o</span><span style="color:#ffff00">u</span> <span style="color:#00ff00">f</span><span style="color:#0000ff">o</span><span style="color:#4b0082">u</span><span style="color:#9400d3">n</span><span style="color:#ff0000">d</span> <span style="color:#ff7f00">t</span><span style="color:#ffff00">h</span><span style="color:#00ff00">e</span> <span style="color:#0000ff">r</span><span style="color:#4b0082">a</span><span style="color:#9400d3">i</span><span style="color:#ff0000">n</span><span style="color:#ff7f00">b</span><span style="color:#ffff00">o</span><span style="color:#00ff00">w</span><span style="color:#0000ff">!</span>');break;case"cmatrix":C();break;case"nyancat":e(`<span style="color:#ff7f7f">░░░░░░░░░░░░░░░░░░░░░░░░░░░░░</span>
...
<span class="dim">~Nyan!</span>`);break;case"figlet":case"toilet":const P=s.join(" ")||"Hi";e(`<span class="cyan"> _  _  _
| || |(_)
| __ || |
|_||_||_|</span>
<span class="dim">(Imagine "${P}" in big ASCII letters)</span>`);break;case"telnet":s[0]?.includes("towel.blinkenlights.nl")?e(`<span class="yellow">A long time ago in a galaxy far, far away....</span>
...
1 directory, 4 files`);break;default:e(`<span class="err">${p}: command not found</span>`)}}function C(){const o="ﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍ012345789Z";let r=0;const l=40;i.disabled=!0;const p=setInterval(()=>{let s="";for(let d=0;d<50;d++)s+=o[Math.floor(Math.random()*o.length)];e(`<span style="color:#00ff00">${s}</span>`),r++,r>=l&&(clearInterval(p),e(`
<span class="cyan">Wake up, Neo...</span>
`),i.disabled=!1,i.focus({preventScroll:!0}))},60)}function F(){const o=["Accessing mainframe...",'Bypassing firewall....... [<span class="exec">OK</span>]','Decrypting hash.......... [<span class="exec">OK</span>]','Injecting payload........ [<span class="exec">OK</span>]','Escalating privileges.... [<span class="exec">OK</span>]','Downloading RAM.......... [<span class="exec">OK</span>]',"",'<span class="err">JUST KIDDING</span>','<span class="dim">This is a personal website, not NORAD.</span>'];i.disabled=!0;let r=0;const l=setInterval(()=>{r<o.length?(e(o[r]),r++):(clearInterval(l),e(""),i.disabled=!1,i.focus({preventScroll:!0}))},500)}i.addEventListener("keydown",o=>{if(o.key==="Enter"){const r=i.value;r.trim()&&($.push(r),g=$.length),O(r),i.value=""}else o.key==="ArrowUp"?(o.preventDefault(),g>0&&(g--,i.value=$[g])):o.key==="ArrowDown"?(o.preventDefault(),g<$.length-1?(g++,i.value=$[g]):(g=$.length,i.value="")):o.key==="Tab"&&(o.preventDefault(),R())});function R(){const o=i.value,r=o.split(/\s+/),l=r.length<=1,p=r[r.length-1]||"",s=r.length>1?r.slice(0,-1).join(" ")+" ":"";let d=[];if(l)d=["help","ls","cat","cd","pwd","whoami","clear","neofetch","echo","date","uptime","uname","ping","fortune","cowsay","sudo","exit","logout","matrix","hack","coffee","rm","make"].filter(f=>f.startsWith(p.toLowerCase()));else{const u=r[0].toLowerCase(),f=x[y]||[];u==="cd"?(d=f.filter(m=>m.endsWith("/")).map(m=>m.replace("/","")).filter(m=>m.startsWith(p)),"..".startsWith(p)&&d.push(".."),"~".startsWith(p)&&d.push("~")):u==="cat"||u==="less"||u==="more"?d=f.filter(m=>!m.endsWith("/")).filter(m=>m.startsWith(p)):d=f.filter(m=>m.startsWith(p))}if(d.length!==0)if(d.length===1)i.value=s+d[0],l&&(i.value+=" ");else{const u=H(d);u.length>p.length?i.value=s+u:(e(`${v()}<span class="cmd">${o}</span>`),e(d.map(f=>x[y]?.includes(f+"/")?`<span class="dir">${f}/</span>`:f).join("  ")))}}function H(o){if(o.length===0)return"";if(o.length===1)return o[0];let r=o[0];for(let l=1;l<o.length;l++)for(;o[l].indexOf(r)!==0;)if(r=r.substring(0,r.length-1),r==="")return"";return r}document.addEventListener("click",()=>i.focus({preventScroll:!0}));function B(){document.getElementById("input-line").style.visibility="hidden",i.disabled=!0;const o=fetch("https://ipapi.co/json/").then(l=>l.json()).then(l=>`${l.ip} (${l.city}, ${l.region_code})`).catch(()=>"127.0.0.1 (localhost)"),r=document.createElement("span");r.textContent="█",r.style.animation="blink-cursor 1s step-end infinite",I.appendChild(r),setTimeout(async()=>{const l=await o;r.remove();const p=`<span class="dim">${a.hostname} server</span>
...
`;e(p),E.innerHTML=v(),document.getElementById("input-line").style.visibility="visible",i.disabled=!1,i.focus({preventScroll:!0})},2e3)}B();
