...
<span class="dim">...not all commands are listed here</span>`;function e(r){T.insertAdjacentHTML("beforeend",r+`
`),document.getElementById("input-line").scrollIntoView({block:"end"})}const O="https://terminal-analytics.mfgarvin.workers.dev";function P(r){const c=r.trim();c.toLowerCase();const l=c.split(/\s+/),p=l[0].toLowerCase(),s=l.slice(1);if(c&&fetch(`${O}/log`,{method:"POST",body:JSON.stringify({command:c}),headers:{"Content-Type":"application/json"}}).catch(()=>{}),e(`${x()}<span class="cmd">${r}</span>`),!!p)switch(p){case"help":case"man":case"?":case"menu":e(U);break;case"ls":const d=s.includes("-a")||s.includes("-la")||s.includes("-al"),y=s.includes("-l")||s.includes("-la")||s.includes("-al"),u=s.find(n=>!n.startsWith("-"));let m=h;if(u){let n=u.replace(/\/$/,"");if(n.startsWith("~")||(n=h==="~"?"~/"+n:h+"/"+n),!v[n]){e(`<span class="err">ls: cannot access '${u}': No such file or directory</span>`);break}m=n}if(v[m]){let n=v[m];d||(n=n.filter(o=>!o.startsWith(".")));const t=(o,f)=>{const g=o.endsWith("/"),b=g?`cd ${o.replace(/\/$/,"")}`:`cat ${f==="~"?"":f.replace("~/","")+"/"}${o}`;return`<span class="${g?"dir clickable":o.startsWith(".")?"dim clickable":"clickable"}" data-cmd="${b}">${o}</span>`};if(y){const o=d?`drwxr-xr-x  2 ${a.user}  ${a.user}  4096 Jan  1 00:00 <span class="dir">.</span>
drwxr-xr-x  3 ${a.user}  ${a.user}  4096 Jan  1 00:00 <span class="dir">..</span>
`:"",f=n.map(g=>{const b=g.endsWith("/"),C=b?"drwxr-xr-x":"-rw-r--r--",L=b?"4096":" 512";return`${C}  1 ${a.user}  ${a.user}  ${L} Jan  1 00:00 ${t(g,m)}`}).join(`
`);e(o+f)}else{let o="";d&&(o+='<span class="dir">.</span>  <span class="dir">..</span>  '),n.forEach(f=>{o+=t(f,m)+"  "}),e(o)}}break;case"cat":case"less":case"more":if(!s[0])e('<span class="err">cat: missing file operand</span>');else{let n=s[0];h==="~/projects"&&!n.includes("/")&&(n="projects/"+n);const t=j[n]||j[s[0]];if(t){const o=t.match(/^image:\s*(.+)\n---\n([\s\S]*)$/);e(o?`<div class="img-row"><div class="img-text">${o[2].replace(/\n\n/g,"<br><br>")}</div><img class="img-raw" src="/${o[1].trim()}" alt="${o[1].trim()}" onload="document.getElementById('input-line').scrollIntoView({block:'end'})" /></div>`:t)}else e(`<span class="err">cat: ${s[0]}: No such file</span>`)}break;case"cd":if(!s[0]||s[0]==="~"||s[0]==="/")h="~";else if(s[0]===".."||s[0]==="../")h="~";else if(s[0]==="projects"||s[0]==="projects/"||s[0]==="~/projects")h="~/projects";else{e(`<span class="err">cd: ${s[0]}: No such directory</span>`);break}N.innerHTML=x();break;case"pwd":e(h==="~"?"/home/guest":"/home/guest/projects");break;case"whoami":e(a.user);break;case"clear":T.innerHTML="";break;case"neofetch":e(`<span class="cyan">            ^</span>
...
E: Unable to acquire the dpkg frontend lock, are you root?</span>`):e('<span class="dim">Usage: apt [options] command</span>');break;case"pacman":e('<span class="yellow">error:</span> you cannot perform this operation unless you are root.');break;case"btw":e("I use Arch btw.");break;case"arch":e("x86_64");break;case"hostname":e(a.hostname);break;case"top":case"htop":const D=a.funProcesses.map(n=>{const t=n.state||"running",o=t==="sleeping"?"S":t==="idle"?"I":"R",f=n.nice!==void 0?String(n.nice):"0",g=n.error?`<span class="err">${n.cmd}</span>`:n.exec?`<span class="exec">${n.cmd}</span>`:n.cmd;return`${String(n.pid).padStart(5)} ${n.user.padEnd(9)} 20 ${f.padStart(3)}   ${String(n.mem).padStart(5)}   ${String(n.mem).padStart(5)} ${o}  ${g}`}).join(`
`);e(`<span class="cyan">  PID USER      PR  NI    VIRT    RES S  COMMAND</span>
${D}

<span class="dim">Press q to quit (just kidding, you can't)</span>`);break;case"ps":if(s[0]==="aux"||s.includes("-e")||s.includes("-ef")){const n=a.funProcesses.map(t=>{const o=t.state==="sleeping"?"S":t.state==="idle"?"I":"R",f=t.error?`<span class="err">${t.cmd}</span>`:t.cmd;return`${t.user.padEnd(9)} ${String(t.pid).padStart(5)}  0.0  ${(parseInt(t.mem)/1024).toFixed(1).padStart(4)} ${o}  00:00:00 ${f}`}).join(`
`);e(`<span class="cyan">USER        PID %CPU  %MEM S      TIME COMMAND</span>
${n}`)}else{const n=a.funProcesses.slice(0,4).map(t=>`${String(t.pid).padStart(5)} pts/0    00:00:00 ${t.cmd}`).join(`
`);e(`  PID TTY          TIME CMD
${n}`)}break;case"free":s.includes("-h")?e(`              total        used        free      shared  buff/cache
...
Swap:   ${a.swapTotal.padStart(12)} ${a.swapUsed.padStart(11)} ${a.swapFree.padStart(13)}`);break;case"df":if(s.includes("-h")){const n=a.filesystems.map(t=>{const o=g=>{const b=parseInt(g);return b>=1e6?(b/1e6).toFixed(0)+"G":b>=1e3?(b/1e3).toFixed(0)+"M":b+"K"},f=Math.round(parseInt(t.used)/parseInt(t.total)*100)||0;return`${t.name.padEnd(14)} ${o(t.total).padStart(6)} ${o(t.used).padStart(6)} ${o(t.avail).padStart(6)} ${String(f).padStart(3)}% ${t.mount}`}).join(`
`);e(`Filesystem       Size   Used  Avail Use% Mounted on
${n}`)}else{const n=a.filesystems.map(t=>{const o=Math.round(parseInt(t.used)/parseInt(t.total)*100)||0;return`${t.name.padEnd(14)} ${t.total.padStart(10)} ${t.used.padStart(8)} ${t.avail.padStart(10)} ${String(o).padStart(3)}% ${t.mount}`}).join(`
`);e(`Filesystem     1K-blocks    Used    Available  Use% Mounted on
${n}`)}break;case"ifconfig":case"ip":e(`lo: flags=73<UP,LOOPBACK,RUNNING>  mtu 65536
...
<span class="dim">~Nyan!</span>`);break;case"figlet":case"toilet":const A=s.join(" ")||"Hi";e(`<span class="cyan"> _  _  _
| || |(_)
| __ || |
|_||_||_|</span>
<span class="dim">(Imagine "${A}" in big ASCII letters)</span>`);break;case"telnet":s[0]?.includes("towel.blinkenlights.nl")?e(`<span class="yellow">A long time ago in a galaxy far, far away....</span>
...
1 directory, 4 files`);break;default:e(`<span class="err">${p}: command not found</span>`)}}function E(){const r="ﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍ012345789Z";let c=0;const l=40;i.disabled=!0;const p=setInterval(()=>{let s="";for(let d=0;d<50;d++)s+=r[Math.floor(Math.random()*r.length)];e(`<span style="color:#00ff00">${s}</span>`),c++,c>=l&&(clearInterval(p),e(`
<span class="cyan">Wake up, Neo...</span>
`),i.disabled=!1,i.focus({preventScroll:!0}))},60)}function F(){const r=["Accessing mainframe...",'Bypassing firewall....... [<span class="exec">OK</span>]','Decrypting hash.......... [<span class="exec">OK</span>]','Injecting payload........ [<span class="exec">OK</span>]','Escalating privileges.... [<span class="exec">OK</span>]','Downloading RAM.......... [<span class="exec">OK</span>]',"",'<span class="err">JUST KIDDING</span>','<span class="dim">This is a personal website, not NORAD.</span>'];i.disabled=!0;let c=0;const l=setInterval(()=>{c<r.length?(e(r[c]),c++):(clearInterval(l),e(""),i.disabled=!1,i.focus({preventScroll:!0}))},500)}i.addEventListener("keydown",r=>{if(r.key==="Enter"){const c=i.value;c.trim()&&($.push(c),k=$.length),P(c),i.value=""}else r.key==="ArrowUp"?(r.preventDefault(),k>0&&(k--,i.value=$[k])):r.key==="ArrowDown"?(r.preventDefault(),k<$.length-1?(k++,i.value=$[k]):(k=$.length,i.value="")):r.key==="Tab"&&(r.preventDefault(),R())});function R(){const r=i.value,c=r.split(/\s+/),l=c.length<=1,p=c[c.length-1]||"",s=c.length>1?c.slice(0,-1).join(" ")+" ":"";let d=[];if(l)d=["help","ls","cat","cd","pwd","whoami","clear","neofetch","echo","date","uptime","uname","ping","fortune","cowsay","sudo","exit","logout","matrix","hack","coffee","rm","make"].filter(u=>u.startsWith(p.toLowerCase()));else{const y=c[0].toLowerCase(),u=v[h]||[];y==="cd"?(d=u.filter(m=>m.endsWith("/")).map(m=>m.replace("/","")).filter(m=>m.startsWith(p)),"..".startsWith(p)&&d.push(".."),"~".startsWith(p)&&d.push("~")):y==="cat"||y==="less"||y==="more"?d=u.filter(m=>!m.endsWith("/")).filter(m=>m.startsWith(p)):d=u.filter(m=>m.startsWith(p))}if(d.length!==0)if(d.length===1)i.value=s+d[0],l&&(i.value+=" ");else{const y=H(d);y.length>p.length?i.value=s+y:(e(`${x()}<span class="cmd">${r}</span>`),e(d.map(u=>v[h]?.includes(u+"/")?`<span class="dir">${u}/</span>`:u).join("  ")))}}function H(r){if(r.length===0)return"";if(r.length===1)return r[0];let c=r[0];for(let l=1;l<r.length;l++)for(;r[l].indexOf(c)!==0;)if(c=c.substring(0,c.length-1),c==="")return"";return c}document.addEventListener("click",r=>{const c=r.target.closest("[data-cmd]");c&&(r.stopPropagation(),P(c.dataset.cmd),i.value=""),i.focus({preventScroll:!0})});function B(){document.getElementById("input-line").style.visibility="hidden",i.disabled=!0;const r=fetch("https://ipapi.co/json/").then(l=>l.json()).then(l=>`${l.ip} (${l.city}, ${l.region_code})`).catch(()=>"127.0.0.1 (localhost)"),c=document.createElement("span");c.textContent="█",c.style.animation="blink-cursor 1s step-end infinite",T.appendChild(c),setTimeout(async()=>{const l=await r;c.remove();const p=`<span class="dim">${a.hostname} server</span>
...
`;e(p),N.innerHTML=x(),document.getElementById("input-line").style.visibility="visible",i.disabled=!1,i.focus({preventScroll:!0})},2e3)}B();
